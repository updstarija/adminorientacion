...
const FilaTable = (props) => {
    ...
    async function _SetEstadoTest(){
      if(telefono=='+52 624 129 2406'){
        ...
        console.log('rev comple ', ' 1',(test?.resumen?.test1 , ' 2',test?.resumen?.test2 , ' 3',test?.resumen?.test3 , ' 4',test?.resumen?.test4) )
      }
      if( test?.resumen?.test1 &&
          test?.resumen?.test2 &&
          test?.resumen?.test3 &&
          test?.resumen?.test4
        ){
    ...
    function _RenderWhatapp(){
      ...
      if(telefono === 'Sin telefono' ){
        ...
        return (<FontAwesomeIcon icon={faWhatsapp} onClick={()=>{EnviarWhatsapp(`${telefono}`,mensajeWhatsapp)}} className='IconoWhatsapp animate__animated animate__swing'/>)
      }
    ...
    const Render_Aprobar = (revisadoPor='' , TestCompleto = false,aprobadoPor ='') => {
      if(revisadoPor.length > 0 && revisadoPor != 'sin revisado' && TestCompleto){
        if(aprobadoPor.length > 0){
          return <ModalAprobarRevision icon='libro' buttonLabel='Rev.T' test={{...test}}/>
        }else{
          return <ModalAprobarRevision icon='manito'   buttonLabel='Rev.T' test={{...test}}/>
        }
      }
    }
    return (
        <Fragment>
            <tr>
              ...
              <td>
                <p onClick={()=>{
                  // ChangeNameUsuario()
                }} className="m-0 p-0 tex">{`${nombres} ${apellidos}`}</p>
                <p className="m-0 p-0 text-muted">{`${colegio}`}  </p>
              </td>
              <td>
                {test?.fechaevaluacion ? <p>{moment(test?.fechaevaluacion).format('DD/MM/YYYY, h:mm:ss a')}</p> : 'no tiene fecha'}
              ...
              <td>{`${revisadoPor}`}</td>
              <td>
                {test?.fechaaprobado ? <p>{moment(new Date(test?.fechaaprobado )).format('DD/MM/YYYY, h:mm:ss a')}</p> : 'no tiene fecha'}
              ...
              <td>
                ...
                {TestCompleto? <ModalRevisarTest Revisado={TestReviado} buttonLabel='Rev.T' test={{...test}}/>: null}
                ...
                {TestCompleto== false ?
                  <FontAwesomeIcon icon={faTrash} onClick={()=>{ShowModalDeleteFila(Id)}} style={{backgroundColor:'rgba 255,80,80,1'}} className='IconoTrash IconosToggleModal'/>
                :null}
